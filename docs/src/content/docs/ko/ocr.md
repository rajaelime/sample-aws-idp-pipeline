...
| 백엔드 | 모델 | 이유 |
|--------|------|------|
| **Lambda (CPU)** | PP-OCRv5, PP-StructureV3 | 경량 모델, GPU 불필요, 빠른 시작 |
...
| 의존성 | `paddleocr>=3.3.0`, `paddlepaddle>=3.2.2`, `boto3` |
| 모델 캐시 | S3에 모델 아카이브 캐싱 (초기 다운로드 후 재사용) |
...
:::note
`ml.g5.xlarge` 기준 온디맨드 비용은 시간당 약 $1.41입니다. Scale-to-zero로 사용한 시간만 과금됩니다. PP-OCRv5/PP-StructureV3는 Lambda에서 실행되므로 SageMaker 비용이 발생하지 않습니다.
:::
...
| 역할 | OCR 추론, S3 결과 저장, DynamoDB 상태 업데이트 |
| 대상 모델 | PP-OCRv5, PP-StructureV3 |
...
| 모델 | 백엔드 | 설명 | 용도 |
|------|--------|------|------|
| **PP-OCRv5** | Lambda (CPU) | 높은 정확도의 범용 텍스트 추출 OCR | 일반 문서, 다국어 텍스트 |
| **PP-StructureV3** | Lambda (CPU) | 테이블 및 레이아웃 감지 포함 문서 구조 분석 | 표, 양식, 복잡한 레이아웃 |
