...
| 백엔드 | 모델 | 이유 |
|--------|------|------|
| **Lambda (CPU)** | PP-OCRv5 | MNN 기반 Rust Lambda 추론, GPU 불필요, 빠른 시작 |
| **SageMaker (GPU)** | PaddleOCR-VL | Vision-Language 모델, GPU 필수 |

PP-OCRv5는 MNN 기반 CPU 추론을 사용하는 Rust Lambda에서 실행되며, 모델이 빌드 시 번들되어 모델 다운로드로 인한 콜드 스타트가 없습니다. PaddleOCR-VL은 텍스트 영역별 VLM 추론에 GPU가 필수적이므로 SageMaker에서 실행합니다.
...
SQS (OCR Queue)
  → Lambda (OCR Invoker) ─── 모델에 따라 라우팅
      │
      ├─ [PP-OCRv5] ── CPU 모델
      │     → Lambda (OCR Processor) ── Python 어댑터
      │         → Lambda (Rust PaddleOCR) ── MNN 추론
      │             ├─ S3 (result.json 저장)
...
2개의 Lambda 아키텍처: OCR Processor(Python)가 Rust PaddleOCR Lambda를 호출하여 추론한 후, 응답을 변환하여 S3와 DynamoDB에 결과를 저장합니다.
...
| 런타임 | Rust (cargo-lambda) |
| 아키텍처 | x86_64 |
| 메모리 | 2048 MB |
| 타임아웃 | 5분 |
| 추론 | MNN 기반 CPU 추론 |
| 모델 | 빌드 시 번들 (런타임 다운로드 없음) |
...
| 메모리 | 256 MB |
| 타임아웃 | 10분 |
| 역할 | Rust OCR Lambda 호출, 응답 변환, 결과 저장 |
...
이러한 제약으로 PP-OCRv5는 Lambda(MNN 기반 Rust Lambda)에서 처리하여 SageMaker 콜드 스타트를 회피하고, VL만 SageMaker에서 실행합니다.
...
| 모델 | 백엔드 | 설명 | 용도 |
|------|--------|------|------|
| **PP-OCRv5** | Lambda (CPU) | MNN 기반 Rust Lambda 추론, 높은 정확도의 텍스트 추출 | 일반 문서, 다국어 텍스트 |
