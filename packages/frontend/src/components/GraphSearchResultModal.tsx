...
export default function GraphSearchResultModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-7xl mx-4 rounded-2xl border border-blue-500/30 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
        style={{ backgroundColor: GRAPH_BG, height: '85vh' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-blue-500/20 bg-gradient-to-r from-blue-900/30 to-indigo-900/30">
        ...
        >
          {/* Left: Answer content */}
          {hasAnswer && (
            <div className="w-2/5 border-r border-blue-500/20 overflow-y-auto p-5">
              {/* Entity chips */}
              {data.entities?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {data.entities.map((entity, i) => (
                    <span
                      key={i}
                      className="text-[10px] font-medium px-2 py-1 rounded-full border border-violet-500/40 bg-violet-900/30 text-violet-300"
                    >
                      {entity.name}
              ...
              {/* Answer markdown */}
              <div className="prose prose-sm prose-invert max-w-none text-slate-200 [&_strong]:!text-inherit [&_p]:leading-relaxed">
          ...
          {/* Right (or full): Graph */}
          <div className={hasAnswer ? 'flex-1' : 'w-full h-full'}>
