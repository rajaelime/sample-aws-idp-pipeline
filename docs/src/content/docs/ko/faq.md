...
OCR Lambda는 대용량 문서 처리를 위해 5,120MB 메모리가 필요합니다. Lambda 메모리는 원래 10,240MB까지 설정 가능하지만, 일부 신규 또는 무료 계정은 기본 할당량이 3,008MB로 제한되어 배포가 실패할 수 있습니다. 이 할당량은 별도 요청이 불가하며, 계정 사용량에 따라 자동으로 증가합니다.

:::note
Service Quotas 대시보드에서 현재 메모리 할당량을 확인하세요. 신규 계정에서는 할당량이 자동 증가할 때까지 OCR 스택 배포가 제한될 수 있습니다.
:::
