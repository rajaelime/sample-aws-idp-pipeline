...
| **Entity** | 抽出されたエンティティ | `id`, `project_id`, `name` |
...
Neptuneはセカンダリインデックスをサポートしておらず、ノードの`~id`プロパティが唯一のO(1)直接検索手段です。そのため、各ノードタイプのIDを意味のある複合キーとして構成し、インデックスなしでも高速な検索を可能にしています。
...
- **Segment/Analysis**: ワークフローID + セグメントインデックス（+ QAインデックス）を組み合わせ、IDだけで所属関係を把握できます
- **Entity**: プロジェクトID + 正規化された名前のハッシュを使用し、同じエンティティが複数のセグメントから抽出されても自然に同一ノードにマージ（MERGE）されます
