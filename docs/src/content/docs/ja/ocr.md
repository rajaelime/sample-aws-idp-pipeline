...
| バックエンド | モデル | 理由 |
|-------------|--------|------|
| **Lambda (CPU)** | PP-OCRv5 | MNNベースのRust Lambda推論、GPU不要、高速起動 |
| **SageMaker (GPU)** | PaddleOCR-VL | Vision-Languageモデル、GPU必須 |

PP-OCRv5はMNNベースのCPU推論を使用するRust Lambdaで実行され、モデルはビルド時にバンドルされるためモデルダウンロードによるコールドスタートがありません。PaddleOCR-VLはテキスト領域ごとのVLM推論にGPUが必須のため、SageMakerで実行します。
...
SQS (OCR Queue)
  → Lambda (OCR Invoker) ─── モデルに応じてルーティング
      │
      ├─ [PP-OCRv5] ── CPUモデル
      │     → Lambda (OCR Processor) ── Pythonアダプター
      │         → Lambda (Rust PaddleOCR) ── MNN推論
      │             ├─ S3 (result.json保存)
      │             └─ DynamoDB (前処理ステータス更新)
      │
      └─ [PaddleOCR-VL] ── GPUモデル
...
2つのLambdaアーキテクチャ：OCR Processor（Python）がRust PaddleOCR Lambdaを呼び出して推論を実行し、レスポンスを変換してS3とDynamoDBに結果を保存します。
...
| ランタイム | Rust (cargo-lambda) |
| アーキテクチャ | x86_64 |
| メモリ | 2048 MB |
| タイムアウト | 5分 |
| 推論 | MNNベースCPU推論 |
| モデル | ビルド時にバンドル（ランタイムダウンロードなし） |
...
| メモリ | 256 MB |
| タイムアウト | 10分 |
| 役割 | Rust OCR Lambda呼び出し、レスポンス変換、結果保存 |
...
これらの制約により、PP-OCRv5はLambda（MNNベースのRust Lambda）で処理してSageMakerコールドスタートを回避し、VLのみSageMakerで実行します。
...
|--------|-------------|------|-------------|
| **PP-OCRv5** | Lambda (CPU) | MNNベースRust Lambda推論、高精度テキスト抽出 | 一般文書、多言語テキスト |
