...
OCR Lambdaは大容量ドキュメント処理のために5,120MBのメモリが必要です。Lambdaメモリは通常10,240MBまで設定可能ですが、一部の新規またはフリーティアアカウントではデフォルトクォータが3,008MBに制限されており、デプロイが失敗する場合があります。このクォータは手動で申請できず、アカウントの使用量に応じて自動的に増加します。

:::note
Service Quotasダッシュボードで現在のメモリクォータを確認してください。新規アカウントでは、クォータが自動増加するまでOCRスタックのデプロイが制限される場合があります。
:::
